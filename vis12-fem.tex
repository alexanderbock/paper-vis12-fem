...
\begin{figure}[b]
  \centering
    \begin{minipage}{1.5cm} 
      \centering 
      \subfigure{\resizebox{1.25cm}{2cm}{\includegraphics{figures/coincidence/HeartLowPrecisionPoints_new.jpg}} }\\
      \subfigure{\resizebox{1.25cm}{2cm}{\includegraphics{figures/coincidence/HeartLowPrecisionSurface_new.jpg}} }  
     \end{minipage}
...
\begin{figure*}[t]
    \centering
    \subfigure{\includegraphics[width=0.15\linewidth]{figures/breast_mesh/entry_point.pdf}}
    \setcounter{subfigure}{0}
    \subfigure[entry world]{\includegraphics[width=0.15\linewidth]{figures/breast_mesh/BreastEntryWorld1}}
    \subfigure[entry $\xi$]{\includegraphics[width=0.15\linewidth]{figures/breast_mesh/BreastEntryXi1} \label{fig:depthpeeling:entryxi}}
    \subfigure[entry depth]{\includegraphics[width=0.15\linewidth]{figures/breast_mesh/BreastEntryDepth1}}
    \subfigure[entry element IDs]{\includegraphics[width=0.15\linewidth]{figures/breast_mesh/BreastElementID1} \label{fig:depthpeeling:entryid}}
    \subfigure[entry face IDs]{\includegraphics[width=0.15\linewidth]{figures/breast_mesh/BreastEntryFace1}}\\
    \subfigure{\includegraphics[width=0.15\linewidth]{figures/breast_mesh/exit_point.pdf}}
...
In this section, we are going to elaborate some of the implementational details regarding the previously mentioned concepts.

\begin{figure*}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/memory_layout.pdf}
    \caption{Memory layout used to gain access to the precomputed proxy ray information. To retrieve the information required during rendering in constant time, a hierarchical subdivision is used. For each proxy ray, we store the cluster ID as well as the rotation angle $\theta$.}
    \label{fig:memory_layout}
\end{figure*}


\noindent \textbf{Proxy Ray Generation.} In order to get an equidistant point distribution on the faces of the elements, we subdivide each element's face in the $\xi$ coordinate system, where it is of a simpler geometric shape and therefore trivial to do. The points in $\xi$ space can be transformed into world space by the formula presented in Section~\ref{sec:theory}. For each pair $p = (p_0, p_1)$ of entry- and exit-points, we can compute the (in world space) straight ray between $p_0$ and $p_1$ and sample it in $s_r$ equidistant positions. Each sample point is transformed from world to $\xi$ space using an iterative Newton method and stored for use as one control point for the Catmull-Rom spline approximating the original ray. Each spline's control points are world space independent and are stored in a structure accessible via the equation provided in Section~\ref{subsec:peeling}.

\noindent \textbf{Curve Similarity.} The first step here is to increase the similarity of the stored splines in such a way, that the set of splines can be represented by a smaller number of clusters. Three transformations are performed after which each spline fulfills three traits: 1. The starting point  $x_0$ is at $(0,0,0)$; 2. The first non-collinear point $x_i$ lies in the $yz$-plane; 3. The end point $x_n$ is at $(0,0,1)$. We apply three matrices $M_t$, $M_{rot}$, and $M_s$ to all control points, where $M_t$ is a translation by $-x_0$, $M_s$ is a scaling by $1 / x_n.z$, and $M_rot$ is a rigid-body transformation that will rotate the points such that $x_n$ is on the $z$ axis andh the non-collinearity is fulfilled. Thus, we get new control points $x' = M_s \cdot M_{rot} \cdot M_t \cdot x$ with a higher degree of similarity. By enforcing that $x_i$ lies in the $yz$-plane, we lose one degree of freedom which is the rotational angle w.r.t. the z-axis. We will store this angle $\theta$ for each spline (compared to for each cluster), thereby making a trade-off between spline similarity and accuracy.

\noindent \textbf{Curve clustering.} In order to reduce the number of splines which need to be stored on the graphics card, we apply a clustering algorithm such that multiple splines are substituted by a single representative. The clustering algorithm we have chosen is k-means, which increases the quality of the representatives with each iteration. Finding a necessary number of iterations a priori is a hard problem, so we investigated multiple tests to check if the sufficient quality has been reached. The simplest test is a fixed number of iterations, which is already a feasible test (see Figure~\ref{fig:clustering_epsilon_error}). Another possible test is the percentage of splines which change clusters in a single iteration; this is based on the assumption that this number decreases asymptotically to 0. A CPU implementation using C++ and OpenMP was used to generate the measurements presented in Section~\ref{sec:results}. The clustering was performed on 8 threads and it was found that the hard disk access as well as the computing power is a bottle neck for faster execution times. An OpenCL-based implementation was tested, that keeps the cluster representatives for the current iteration in local memory and fetches the splines from global memory. Depending on various parameters, e.g.\,work sizes, number of parallel cluster calculations, we achieved a speedup in the area of one order of magnitude compared to the CPU implementation.

\noindent \textbf{Ray Marching.} The rendering step consists of a depth peeling step to get the entry- and exit points for each ray which is cast. The geometry representation of the FE models contains 4 values for each node; the three color channels provide the $\xi$ value (see Figures~\ref{fig:depthpeeling:entryxi}) and the alpha channel stores the element for which the nodal value is defined (see Figure~\ref{fig:depthpeeling:entryid}). Based on these information, all free parameters for the equation provided in Section~\ref{subsec:peeling} can be derived: In $\xi$ space, the reference object is a cube, and both entry- and exit points reside on a side of the cube. Thus, one of the $\xi$ components will be 0 or 1, and we can derive the face id from the combination of the two variables. With all information present, the cluster is fetched for this entry-exit point pair and the ray marching is performed on the spline.
...
\begin{table}[b]
  \caption{Memory consumption of data structures used during rendering for different grid resolutions and with a variation of number of clusters in the case of the heart dataset.}
  \label{tab:data_size}
  \begin{center}
    \begin{tabular}{|l|c|c|c|c|c|c|}
      ...
      \hline
      3 	&	3	&	 2.7\,MB	&	0.39\,MB		&	0.47\,MB	&	0.82\,MB  \\
      7 	&	5 	&	40.7\,MB	&	5.4\,MB		&	5.5\,MB	&	5.85\,MB  \\
      8 	&	8 	&	135\,MB	&	18\,MB		&	18.1\,MB	&	18.5\,MB	\\
      10 	&	10 	&	329\,MB	&	43.9\,MB		&	44.0\,MB	&	44.1\,MB	\\
      15	&	15	&	1.62\,GB	&	222.5\,MB		&	222.6\,MB	&	222.9\,MB \\
